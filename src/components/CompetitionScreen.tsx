    ...
    <div className="relative mx-auto flex w-full max-w-7xl flex-1 flex-col p-3 sm:p-6 pb-20 select-none text-slate-100">
      ...
      {subTab === 'modes' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          {/* Mode 2: Ranked 2v2 */}
          <div className="group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-900 via-slate-900/90 to-blue-950/50 p-5 sm:p-6 shadow-xl transition-all duration-300 hover:border-blue-500/60 hover:shadow-[0_0_30px_rgba(59,130,246,0.25)] hover:-translate-y-1">
            <div className="absolute -right-8 -top-8 h-32 w-32 rounded-full bg-blue-500/10 blur-xl group-hover:bg-blue-500/20 transition-colors" />

            <div>
              <div className="flex items-center justify-between mb-3">
                <span className="rounded-full bg-blue-500/20 border border-blue-500/40 px-3 py-0.5 text-[10px] font-black tracking-widest text-blue-300 uppercase font-display">
                  ÉQUIPE 2v2 CLASSÉ
                </span>
                <span className="text-xs font-mono text-slate-400 flex items-center gap-1">
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <span className="text-xs text-blue-400 font-rajdhani font-bold">Vous + Bot Allié vs 2 Bots</span>
                </div>
              </div>

              <p className="text-xs text-slate-300 font-rajdhani leading-relaxed mt-2">
                Affrontez 2 adversaires en équipe avec votre bot coéquipier. Coordonnez vos passes et défendez votre cage pour progresser dans les rangs compétitifs 2v2.
              </p>

              <div className="mt-4 grid grid-cols-3 gap-2 rounded-2xl bg-slate-950/70 p-2.5 border border-slate-800/80 text-center font-rajdhani">
                <div>
                  <div className="text-[10px] text-slate-500 font-bold uppercase">Format</div>
                  <div className="font-bold text-xs text-slate-200">Équipe 2v2</div>
                </div>
                <div>
                  <div className="text-[10px] text-slate-500 font-bold uppercase">Chrono</div>
                  <div className="font-bold text-xs text-slate-200">2:00 + OT</div>
                ...
                <div>
                  <div className="text-[10px] text-slate-500 font-bold uppercase">Gain Victoire</div>
                  <div className="font-bold text-xs text-emerald-400">+30 à +40 MMR</div>
                </div>
              </div>
            </div>

            <button
              id="btn-play-ranked-2v2"
              onClick={() => handleStartSearch('ranked-2v2')}
              className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 py-3 font-display text-sm font-black italic tracking-wide text-white shadow-[0_0_20px_rgba(59,130,246,0.4)] transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
              <Users className="h-4 w-4" />
              <span>RECHERCHER UN MATCH 2v2</span>
              <ArrowRight className="h-4 w-4" />
            </button>
          </div>

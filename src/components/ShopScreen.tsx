...
export const ShopScreen: React.FC<ShopScreenProps> = ({
  ...
  const handleOpenDrop = (drop: LootDrop) => {
    ...
    // Pick random reward
    const randomIdx = Math.floor(Math.random() * possibleRewardItems.length);
    ...
    // 2.2 seconds timeout for smooth crate expansion Framer Motion animation
    setTimeout(() => {
      setWonItem(selectedReward);
      setOpeningStage('revealed');
      try {
        triggerShopChime(selectedReward.rarity);
      } catch {
        // audio play is optional
      }
      try {
        confetti({
          particleCount: 120,
  ...
  return (
    <main className="flex flex-1 flex-col items-center w-full max-w-6xl py-2 text-left relative">
                ...
                >
                  ...
                  <div className="mt-5 pt-3 border-t border-slate-800 flex items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      className="rounded-xl px-4 py-2 bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500 hover:brightness-110 text-slate-950 font-display text-xs font-black tracking-wide shadow-md shadow-amber-500/20 active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer"
                    >
      ...
      {/* ========================================================================= */}
      {openingDrop && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/95 backdrop-blur-md p-4 animate-fade-in">
          <div className="relative w-full max-w-lg rounded-3xl border border-amber-500/40 bg-gradient-to-b from-slate-900 via-slate-950 to-slate-950 p-6 text-center shadow-[0_0_50px_rgba(245,158,11,0.2)] overflow-hidden">
            ...
            <div
              className="absolute inset-0 bg-radial-gradient opacity-30 pointer-events-none"
              style={{

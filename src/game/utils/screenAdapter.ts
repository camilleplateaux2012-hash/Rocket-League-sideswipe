...
class ScreenAdapterService {
  ...
  public async requestFullscreen(targetElement?: HTMLElement | null): Promise<boolean> {
    ...
    try {
      ...
      } else if ((screen as unknown as { lockOrientation?: (mode: string) => void }).lockOrientation) {
        (screen as unknown as { lockOrientation: (mode: string) => void }).lockOrientation("landscape");
      }
    } catch {
      // Safe fallback
    }

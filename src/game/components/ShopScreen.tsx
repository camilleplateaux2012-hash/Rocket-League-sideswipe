...
import { Coins, ShoppingBag, Gift, Sparkles, Check, Star, Eye, CheckCircle2, KeyRound } from 'lucide-react';
...
export const ShopScreen: React.FC<ShopScreenProps> = ({
  ...
  const [duplicateRefund, setDuplicateRefund] = useState<number | null>(null);
  ...
  const handleBuyDirectItem = (item: ShopItem) => {
    ...
    const updatedProfile = unlockItem(item, {
      ...playerProfile,
      coins: coinsAfter,
    });
  ...
  const handleOpenDrop = (drop: LootDrop) => {
    ...
    const selectedReward =
      possibleRewardItems[Math.floor(Math.random() * Math.max(1, possibleRewardItems.length))] ||
      ALL_SHOP_ITEMS[0];

    const isDuplicate = isItemUnlocked(selectedReward);

    pendingRewardRef.current = selectedReward;
    setWonItem(selectedReward);
    setOpeningDrop(drop);
    setOpeningStage('shaking');

    const coinsAfterDeduction = Math.max(0, userCoins - drop.cost);
    let finalProfile: PlayerProfile;

    if (isDuplicate) {
      // Calculate substitution coins (refund value based on item value or crate cost)
      const refundAmount = Math.max(Math.round(selectedReward.cost * 0.75), Math.round(drop.cost * 0.8));
      setDuplicateRefund(refundAmount);

      finalProfile = {
        ...playerProfile,
        coins: coinsAfterDeduction + refundAmount,
      };
    } else {
      setDuplicateRefund(null);
      const profileWithDeduction = {
        ...playerProfile,
        coins: coinsAfterDeduction,
      };
      finalProfile = unlockItem(selectedReward, profileWithDeduction);
    }
  ...
  return (
    <main className="flex flex-1 flex-col items-center w-full max-w-6xl py-2 text-left relative">
            ...
            {LOOT_DROPS.map((drop) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-br ${drop.gradient} opacity-[0.02] group-hover:opacity-[0.06] transition-opacity pointer-events-none`} />
                  ...
                  <div className="mt-5 pt-3 border-t border-slate-800 flex items-center justify-between gap-2">
                    ...
                    <button
                      type="button"
                      data-testid={`open-loot-${drop.id}`}
                      disabled={!canAfford}
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        if (!canAfford) {
                          setErrorMessage("Pièces insuffisantes pour ouvrir ce butin ! Gagnez des matchs pour obtenir plus de pièces.");
                          return;
                        }
                        handleOpenDrop(drop);
                      }}
                      className={`relative z-10 rounded-xl px-4 py-2.5 min-h-11 font-display text-xs font-black tracking-wide shadow-md transition-all flex items-center gap-1.5 select-none touch-manipulation ${
                        canAfford
                          ? 'bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500 hover:brightness-110 text-slate-950 shadow-amber-500/20 active:scale-95 cursor-pointer'
                          : 'bg-slate-800 text-slate-500 border border-slate-700/50 cursor-not-allowed opacity-60'
                      }`}
                    >
                      <Sparkles className="h-3.5 w-3.5 text-slate-950" />
                      <span>{canAfford ? 'OUVRIR' : 'PIÈCES INSUFFISANTES'}</span>
                    </button>
                  </div>
                </div>
              );
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="relative group/canvas my-2 overflow-hidden rounded-2xl">
                      ...
                      <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 flex items-center justify-center rounded-2xl backdrop-blur-[1px] transition-all pointer-events-none">
      ...
      {/* ========================================================================= */}
      {openingDrop && typeof document !== 'undefined'
        ? createPortal(
            <div
              data-testid="loot-reveal-overlay"
              className="fixed inset-0 z-[99999] flex items-center justify-center bg-slate-950/95 p-4"
              style={{ position: 'fixed', inset: 0, zIndex: 99999 }}
              role="dialog"
              aria-modal="true"
              aria-label="Ouverture du butin"
            >
              <div className="relative w-full max-w-lg rounded-3xl border border-amber-500/40 bg-gradient-to-b from-slate-900 via-slate-950 to-slate-950 p-6 text-center shadow-[0_0_50px_rgba(245,158,11,0.2)] overflow-hidden">
                ...
                {openingStage === 'shaking' ? (
                  <div className="py-14 flex flex-col items-center justify-center relative min-h-[320px]">
                    ...
                    <div className="relative z-10 my-6 flex flex-col items-center">
                      <img
                        ...
                        className="h-28 w-28 object-cover rounded-3xl border-2 border-amber-300 shadow-[0_0_60px_rgba(245,158,11,0.9)] animate-crate-grow"
                      />
                    </div>
                    <div className="relative z-10 mt-8 flex flex-col items-center">
                      <div className="h-3 w-48 rounded-full bg-slate-950 overflow-hidden border border-amber-500/40 p-0.5">
                        <div className="h-full w-full origin-left bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-300 rounded-full animate-pulse" />
                      </div>
                      <h3 className="font-display text-lg sm:text-xl font-black italic tracking-wide text-amber-300 mt-4 uppercase">
                        La caisse s'ouvre...
                      </h3>
                      <p className="text-xs text-amber-400/90 font-rajdhani font-bold mt-0.5 tracking-wider uppercase">
                        Obtention de votre objet
                      </p>
                    </div>
                  </div>
                ) : wonItem ? (
                  <div className="py-4 flex flex-col items-center relative z-10 animate-pop-in">
                    <div className="relative z-10 mb-2">
                      <span
                        className={`text-xs font-black uppercase tracking-widest px-3.5 py-1 rounded-full bg-slate-950 border ${wonItem.rarityBorder} ${wonItem.rarityColor} shadow-lg`}
                      >
                        {wonItem.rarity.toUpperCase()} • {wonItem.typeLabel.toUpperCase()}
                      </span>
                    </div>
                    <div className="relative z-10 my-3 w-full max-w-xs overflow-hidden rounded-3xl border border-slate-700/80 bg-slate-950 p-3">
                      <ShopItemVisual item={wonItem} className="h-36 w-full" />
                    </div>
                    <h3 className="relative z-10 font-display text-2xl sm:text-3xl font-black italic text-white tracking-wide mt-1">
                      {wonItem.name}
                    </h3>
                    {duplicateRefund !== null ? (
                      <>
                        <div className="relative z-10 mt-2 flex items-center justify-center gap-1.5 rounded-xl bg-amber-950/90 border border-amber-500/60 px-4 py-2 text-xs font-bold font-rajdhani text-amber-300 shadow-[0_0_15px_rgba(245,158,11,0.2)]">
                          <Coins className="h-4 w-4 text-amber-400 shrink-0" />
                          <span>OBJET DÉJÀ POSSÉDÉ ! COMPENSATION : +{duplicateRefund} PIÈCES !</span>
                        </div>
                        <p className="relative z-10 text-xs text-slate-300 font-rajdhani mt-3 max-w-sm text-center">
                          Vous possédiez déjà <strong className="text-white">{wonItem.name}</strong>. Une substitution de <strong className="text-amber-300">+{duplicateRefund} pièces d'or</strong> a été ajoutée à votre solde !
                        </p>
                      </>
                    ) : (
                      <>
                        <div className="relative z-10 mt-2 flex items-center justify-center gap-1.5 rounded-xl bg-emerald-950/90 border border-emerald-500/60 px-4 py-1.5 text-xs font-bold font-rajdhani text-emerald-300">
                          <CheckCircle2 className="h-4 w-4 text-emerald-400 shrink-0" />
                          <span>Ajouté au garage</span>
                        </div>
                        <p className="relative z-10 text-xs text-slate-400 font-rajdhani mt-3 max-w-sm text-center">
                          Cet objet est disponible dans le Garage et marqué comme possédé dans la boutique.
                        </p>
                      </>
                    )}
                    <div className="relative z-10 mt-6 flex flex-col sm:flex-row gap-3 w-full">
                      <button
                        ...
                        className="flex-1 min-h-11 rounded-2xl bg-slate-800 hover:bg-slate-700 px-5 py-3 font-display text-xs font-black tracking-wide text-white border border-slate-700"
                      >
                        Voir le garage
                      </button>
                      <button
                        ...
                        className="flex-1 min-h-11 rounded-2xl bg-gradient-to-r from-amber-400 via-orange-400 to-amber-500 px-5 py-3 font-display text-xs font-black tracking-wide text-slate-950"
                      >
                        Continuer
                      </button>
                    </div>
                  ...
                  <div className="py-14 flex flex-col items-center justify-center min-h-[240px]">
                    <p className="text-amber-300 font-display text-lg font-black">Chargement du butin...</p>
                    <button
                      type="button"
                      onClick={closeRevealModal}
                      className="mt-6 min-h-11 rounded-xl bg-slate-800 px-4 py-2 text-xs font-bold text-white"
                    >
                      Fermer
                    </button>
                  </div>

...
export const TouchControls: React.FC<TouchControlsProps> = ({
  ...
  const handleZonePointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      setJoystickCenter(initialPos);
      joystickCenterRef.current = initialPos;
  ...
  const handleZonePointerMove = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (!isSteering || steerPointerIdRef.current !== e.pointerId || !center) return;
      e.preventDefault();
      ...
      let currentCenterX = center.x;
      let currentCenterY = center.y;

      // Re-centering (follow-finger) logic: if the thumb drags beyond maxRadius,
      // shift the joystick center in that direction so direction reversals are instant.
      if (dist > maxRadius) {
        const excess = dist - maxRadius;
        currentCenterX += Math.cos(angle) * excess;
        currentCenterY += Math.sin(angle) * excess;
        
        const newCenter = { x: currentCenterX, y: currentCenterY };
        setJoystickCenter(newCenter);
        joystickCenterRef.current = newCenter;
      }

      // Stick visual offset (restricted to maxRadius)
      const clampedDist = Math.min(dist, maxRadius);
      const nx = Math.cos(angle) * clampedDist;
      const ny = Math.sin(angle) * clampedDist;
      setStickOffset({ x: nx, y: ny });

      // Highly precise deadzone calculation with smooth linear scaling
      const deadzone = 5;
      let steerX = 0;
      let steerY = 0;

      if (clampedDist > deadzone) {
        const scale = (clampedDist - deadzone) / (maxRadius - deadzone);
        steerX = Math.cos(angle) * scale;
        steerY = Math.sin(angle) * scale;
      }
      ...
      <div className="pointer-events-auto absolute bottom-3 right-3 sm:bottom-5 sm:right-6 flex flex-col items-end gap-2 pb-[env(safe-area-inset-bottom)] pr-[env(safe-area-inset-right)]">
        ...
        <div className="flex items-center gap-2 pr-1 sm:pr-2">
          ...
          <button
            ...
            className="flex h-12 w-12 sm:h-14 sm:w-14 touch-none flex-col items-center justify-center rounded-xl border border-slate-700 bg-slate-900/85 text-slate-300 shadow-xl backdrop-blur-md active:scale-90 active:border-cyan-400 active:text-cyan-400"
            title="Air Roll 180°"
          >
            <RefreshCw className="h-4 w-4 sm:h-5 sm:w-5" />
            <span className="font-display text-[8px] sm:text-[9px] font-bold">180°</span>
          </button>
          ...
          <button
            ...
            className="flex h-13 w-13 sm:h-16 sm:w-16 touch-none flex-col items-center justify-center rounded-xl sm:rounded-2xl border-2 border-fuchsia-400 bg-gradient-to-br from-indigo-600 to-fuchsia-600 text-white shadow-[0_0_15px_rgba(217,70,239,0.5)] backdrop-blur-md active:scale-90 active:from-indigo-500 active:to-fuchsia-500"
            title="Fast Aerial (Saut + Boost simultané)"
          >
            <Zap className="h-5 w-5 sm:h-6 sm:w-6 fill-current" />
            <span className="font-display text-[8px] sm:text-[9px] font-black tracking-wider uppercase">
              Aerial
            </span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <button
            ...
            className="flex h-16 w-16 sm:h-22 sm:w-22 touch-none flex-col items-center justify-center rounded-full border-2 border-blue-400 bg-blue-600/90 text-white shadow-[0_0_20px_rgba(59,130,246,0.6)] backdrop-blur-md active:scale-90 active:bg-blue-500"
          >
            <ArrowUp className="h-6 w-6 sm:h-8 sm:w-8" />
            <span className="font-display text-[9px] sm:text-[11px] font-extrabold tracking-wider">SAUT</span>
          </button>
          ...
          <button
            ...
            className="flex h-20 w-20 sm:h-26 sm:w-26 touch-none flex-col items-center justify-center rounded-full border-2 border-orange-400 bg-orange-600/90 text-white shadow-[0_0_25px_rgba(249,115,22,0.7)] backdrop-blur-md active:scale-90 active:bg-orange-500"
          >
            <Flame className="h-8 w-8 sm:h-10 sm:w-10 fill-current animate-pulse" />
            <span className="font-display text-[10px] sm:text-[12px] font-black tracking-widest">BOOST</span>
          </button>
        </div>
      </div>
    </div>

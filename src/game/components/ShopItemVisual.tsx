...
function drawDecalItem(ctx: CanvasRenderingContext2D, cx: number, cy: number, decalId: string, time: number) {
  ...
  // Sleek dark steel gray as a canvas body
  const bodyColor = '#1e293b'; 
  let accentColorToUse = '#38bdf8'; 

  if (decalId === 'stripes') accentColorToUse = '#38bdf8';
  else if (decalId === 'flames') accentColorToUse = '#f97316';
  else if (decalId === 'lightning') accentColorToUse = '#22d3ee';
  else if (decalId === 'tribal') accentColorToUse = '#f43f5e';
  else if (decalId === 'stars') accentColorToUse = '#facc15';
  ...
  // Use the exact same detailed decal drawing logic!
  drawCarDecal(ctx, 'octane', decalId, halfW, halfH, accentColorToUse);

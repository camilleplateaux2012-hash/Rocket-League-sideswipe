...
import { Ball, Car, CameraFramingMode, Particle, SpecialShotType, WheelType, CarModel } from '../types/game';
...
export class GameRenderer {
  ...
  public updateCamera(
    ...
    isGoalScored: boolean = false,
    goalScoringTeam?: Team
  ) {
    ...
    // Camera target center (goal focus or always towards player & action)
    let targetCenterX: number;
    let targetCenterY: number;

    if (isGoalScored && goalScoringTeam) {
      // Camera arrives and focuses on the goal where the ball went in
      targetCenterX = goalScoringTeam === 'blue' ? ARENA.rightWallX - 120 : ARENA.leftWallX + 120;
      targetCenterY = ARENA.groundY - 160;
    } else {
      const playerCar = cars[0];
      const playerX = playerCar ? playerCar.x : (minX + maxX) / 2;
      const playerY = playerCar ? playerCar.y : (minY + maxY) / 2;

      const actionCenterX = (minX + maxX) / 2;
      const actionCenterY = (minY + maxY) / 2;
      // Always towards the player (blending action center with player position)
      targetCenterX = actionCenterX * 0.35 + playerX * 0.65;
      targetCenterY = actionCenterY * 0.35 + playerY * 0.65;
    }
  ...
  private drawCar(ctx: CanvasRenderingContext2D, car: Car) {
    ...
    // 7. Player / Bot Name Tag above car roof
    ctx.save();
    ctx.translate(0, -car.height * 0.85 - 16);
    ctx.scale(facing, 1);
    const isBlue = car.team === 'blue';
    const tagBg = isBlue ? 'rgba(30, 58, 138, 0.88)' : 'rgba(124, 45, 18, 0.88)';
    const tagBorder = isBlue ? '#60a5fa' : '#fb923c';

    ctx.font = 'bold 12px Rajdhani, sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';

    const textMetrics = ctx.measureText(car.name);
  ...
  private drawCarBodyVector(ctx: CanvasRenderingContext2D, car: Car, isGhost: boolean) {
    ...
    // Draw customized decals/stickers for the car (unique per model!)
    const decal = car.customization.selectedDecal || 'none';
    if (decal !== 'none') {
  ...
  private drawParticles(ctx: CanvasRenderingContext2D, particles: Particle[], layer: 'under' | 'over') {
    ...
    for (const p of particles) {
      ...
      if (p.type === 'shockwave') {
        ...
        ctx.stroke();
      } else if (p.type === 'solar-ray') {
        const progress = p.life / p.maxLife; // 0 to 1
        const r = 10 + progress * 160; // Expanding radius
        ctx.translate(p.x, p.y);
        ctx.globalAlpha = p.alpha * (1 - progress); // Fade out

        // Solar Rays
        ctx.save();
        ctx.rotate(p.life * 4.5); // Rotating rays based on elapsed life
        ...
        ctx.fillStyle = '#fef08a';
        ctx.beginPath();
        ctx.arc(0, 0, Math.max(5, 30 * (1 - progress)), 0, Math.PI * 2);
        ctx.fill();
      } else if (p.type === 'electro-lightning') {
        const progress = p.life / p.maxLife; // 0 to 1
        const r = 12 + progress * 170; // Expanding radius
        ctx.translate(p.x, p.y);
        ctx.globalAlpha = p.alpha * (1 - progress); // Fade out
        ...
        ctx.shadowBlur = 20;
        for (let i = 0; i < 6; i++) {
          const angle = (p.life * 14 + i * 1.05) % (Math.PI * 2);
          ctx.beginPath();
          ctx.moveTo(0, 0);
        ...
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.arc(0, 0, Math.max(5, 25 * (1 - progress)), 0, Math.PI * 2);
        ctx.fill();
      } else if (p.type === 'supernova-spiral') {
        const progress = p.life / p.maxLife; // 0 to 1
        const r = 8 + progress * 180; // Expanding radius
        ctx.translate(p.x, p.y);
        ctx.globalAlpha = p.alpha * (1 - progress); // Fade out

        // Spiral Nebula Arms
        ctx.save();
        ctx.rotate(p.life * -6.5); // Rotate based on life
        ctx.fillStyle = '#ec4899';
        ctx.shadowColor = '#d946ef';
        ctx.shadowBlur = 15;
        for (let i = 0; i < 24; i++) {
          const a = i * 0.4;
          const dist = a * (1 + progress * 6);
          ctx.fillRect(Math.cos(a) * dist * 3.5, Math.sin(a) * dist * 3.5, 4, 4);
        }
        ctx.restore();
        ...
        ctx.beginPath();
        ctx.arc(0, 0, Math.max(3, 20 * (1 - progress)), 0, Math.PI * 2);
        ctx.fill();
...
/**
 * Draws tailor-made, high-fidelity decals on cars tailored uniquely to each car's shape.
 */
  ...
  switch (model) {
    case 'octane': {
      if (decal === 'stripes') {
        ...
        ctx.lineWidth = 3.5;
        ctx.beginPath();
        // Custom Octane angled dual racing stripes
        ctx.moveTo(-halfW * 0.5, -halfH * 0.35);
        ctx.lineTo(halfW * 0.45, -halfH * 0.1);
        ctx.moveTo(-halfW * 0.5, -halfH * 0.1);
        ctx.lineTo(halfW * 0.45, halfH * 0.1);
        ctx.stroke();
      } else if (decal === 'flames') {
        const grad = ctx.createLinearGradient(-halfW * 0.2, 0, halfW * 0.8, 0);
        grad.addColorStop(0, '#ef4444');
        grad.addColorStop(0.5, '#f97316');
        grad.addColorStop(1, '#eab308');
        ctx.fillStyle = grad;
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.25, halfH * 0.25);
        ctx.quadraticCurveTo(halfW * 0.2, -halfH * 0.25, halfW * 0.5, -halfH * 0.1);
        ctx.quadraticCurveTo(halfW * 0.3, halfH * 0.1, halfW * 0.7, halfH * 0.2);
        ctx.quadraticCurveTo(halfW * 0.3, halfH * 0.35, halfW * 0.5, halfH * 0.45);
        ctx.quadraticCurveTo(0, halfH * 0.5, -halfW * 0.25, halfH * 0.25);
        ctx.fill();
        ...
        ctx.lineTo(-halfW * 0.1, -halfH * 0.15);
        ctx.lineTo(-halfW * 0.25, 0);
        ctx.lineTo(halfW * 0.2, halfH * 0.15);
        ctx.lineTo(0, halfH * 0.25);
        ctx.lineTo(halfW * 0.45, halfH * 0.4);
        ctx.stroke();
      } else if (decal === 'tribal') {
        ctx.strokeStyle = '#020617';
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.5, halfH * 0.1);
        ctx.bezierCurveTo(-halfW * 0.1, -halfH * 0.6, halfW * 0.1, halfH * 0.6, halfW * 0.6, 0);
        ctx.stroke();
      } else if (decal === 'stars') {
        ...
        const drawStar = (x: number, y: number, r: number) => {
          ...
          for (let i = 0; i < 5; i++) {
            ...
            ctx.lineTo(x + Math.cos((54 + i * 72) * Math.PI / 180) * (r / 2), y - Math.sin((54 + i * 72) * Math.PI / 180) * (r / 2));
          }
          ctx.closePath();
          ctx.fill();
        };
        drawStar(halfW * 0.25, -halfH * 0.1, 4.5);
        drawStar(-halfW * 0.2, -halfH * 0.4, 3.5);
        drawStar(0, halfH * 0.3, 3);
      }
      break;
    }

    case 'fennec': {
      // Custom Fennec decals (angular, rally style)
      if (decal === 'stripes') {
        ctx.fillStyle = accentColor;
        ctx.beginPath();
        // Thick rally side diagonal stripe
        ctx.moveTo(-halfW * 0.4, halfH * 0.5);
        ctx.lineTo(-halfW * 0.1, halfH * 0.5);
        ctx.lineTo(halfW * 0.3, -halfH * 0.1);
        ctx.lineTo(halfW * 0.1, -halfH * 0.1);
        ctx.closePath();
        ctx.fill();

        // Plus dual thin hood pinstripes
        ctx.strokeStyle = accentColor;
        ctx.lineWidth = 1.8;
        ctx.beginPath();
        ctx.moveTo(halfW * 0.45, -halfH * 0.12);
        ctx.lineTo(halfW * 0.85, halfH * 0.05);
        ctx.moveTo(halfW * 0.45, -halfH * 0.02);
        ctx.lineTo(halfW * 0.85, halfH * 0.15);
        ctx.stroke();
      } else if (decal === 'flames') {
        // Pixelated/cyber pixel-art fire flames for Fennec
        ctx.fillStyle = '#ef4444';
        ctx.fillRect(-halfW * 0.4, halfH * 0.2, 10, 8);
        ctx.fillRect(-halfW * 0.2, halfH * 0.1, 14, 12);
        ctx.fillStyle = '#f97316';
        ctx.fillRect(-halfW * 0.1, halfH * 0.15, 12, 10);
        ctx.fillRect(halfW * 0.1, halfH * 0.22, 15, 8);
        ctx.fillStyle = '#eab308';
        ctx.fillRect(halfW * 0.25, halfH * 0.25, 18, 6);
      } else if (decal === 'lightning') {
        // Double zig zag parallel bolt on Fennec roof & door
        ctx.strokeStyle = '#22d3ee';
        ctx.shadowColor = '#06b6d4';
        ctx.shadowBlur = 8;
        ctx.lineWidth = 2.5;
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.6, -halfH * 0.8);
        ctx.lineTo(-halfW * 0.4, -halfH * 0.6);
        ...
        ctx.lineTo(0, halfH * 0.1);
        ctx.lineTo(-halfW * 0.1, halfH * 0.2);
        ctx.lineTo(halfW * 0.3, halfH * 0.3);
        ctx.stroke();
      } else if (decal === 'tribal') {
        // Geometric sharp tribal spikes fitting Fennec's angular layout
        ctx.fillStyle = '#0f172a';
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.6, halfH * 0.5);
        ctx.lineTo(-halfW * 0.2, halfH * 0.25);
        ctx.lineTo(-halfW * 0.3, halfH * 0.22);
        ctx.lineTo(halfW * 0.1, -halfH * 0.05);
        ctx.lineTo(-halfW * 0.1, halfH * 0.1);
        ...
        ctx.fillStyle = '#fb7185'; // Cute neon pink stars for rally
        ctx.shadowColor = '#f43f5e';
        ctx.shadowBlur = 6;
        const drawStar = (x: number, y: number, r: number) => {
          ctx.beginPath();
        ...
        drawStar(-halfW * 0.4, -halfH * 0.6, 4.5);
        drawStar(-halfW * 0.1, -halfH * 0.4, 4);
        drawStar(halfW * 0.25, halfH * 0.25, 3.5);
      }
      break;
    }

    case 'dominus': {
      // Custom Dominus decals (muscle drag stripes and long panels)
      if (decal === 'stripes') {
        ctx.fillStyle = accentColor;
        // Thick centered racing stripe and thin offset stripes
        ctx.fillRect(-halfW * 0.8, -halfH * 0.05, halfW * 1.6, 5);
        ctx.fillRect(-halfW * 0.8, -halfH * 0.15, halfW * 1.6, 2);
        ctx.fillRect(-halfW * 0.8, halfH * 0.05, halfW * 1.6, 2);
      } else if (decal === 'flames') {
        const grad = ctx.createLinearGradient(-halfW * 0.8, 0, halfW * 0.4, 0);
        grad.addColorStop(0, '#f43f5e');
        grad.addColorStop(0.5, '#f97316');
        grad.addColorStop(1, '#facc15');
        ctx.fillStyle = grad;
        ctx.beginPath();
        // Huge horizontal muscle car side flames
        ctx.moveTo(-halfW * 0.7, halfH * 0.3);
        ctx.quadraticCurveTo(-halfW * 0.2, -halfH * 0.3, halfW * 0.2, halfH * 0.1);
        ctx.quadraticCurveTo(-halfW * 0.1, halfH * 0.25, halfW * 0.4, halfH * 0.2);
        ctx.quadraticCurveTo(0, halfH * 0.45, halfW * 0.3, halfH * 0.45);
        ctx.quadraticCurveTo(-halfW * 0.3, halfH * 0.55, -halfW * 0.7, halfH * 0.3);
        ctx.closePath();
        ctx.fill();
      } else if (decal === 'lightning') {
        // Lightning bolt spanning Dominus' extremely long body
        ctx.strokeStyle = '#38bdf8';
        ctx.shadowColor = '#0ea5e9';
        ctx.shadowBlur = 10;
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.8, -halfH * 0.1);
        ctx.lineTo(-halfW * 0.4, -halfH * 0.1);
        ctx.lineTo(-halfW * 0.5, halfH * 0.15);
        ctx.lineTo(-halfW * 0.1, halfH * 0.1);
        ctx.lineTo(-halfW * 0.2, halfH * 0.35);
        ctx.lineTo(halfW * 0.3, halfH * 0.3);
        ctx.lineTo(halfW * 0.1, halfH * 0.55);
        ctx.lineTo(halfW * 0.6, halfH * 0.5);
        ctx.stroke();
      } else if (decal === 'tribal') {
        // Sleek horizontal lowrider tribal lines
        ctx.strokeStyle = '#020617';
        ctx.lineWidth = 3.2;
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.7, halfH * 0.45);
        ctx.bezierCurveTo(-halfW * 0.2, -halfH * 0.2, halfW * 0.1, halfH * 0.6, halfW * 0.7, halfH * 0.25);
        ctx.stroke();
      } else if (decal === 'stars') {
        ctx.fillStyle = '#ffffff';
        ...
        drawStar(-halfW * 0.6, -halfH * 0.05, 5);
        drawStar(-halfW * 0.2, -halfH * 0.05, 4.5);
        drawStar(halfW * 0.2, halfH * 0.1, 4);
        drawStar(halfW * 0.5, halfH * 0.12, 4);
      }
      break;
    }

    case 'merc': {
      // Custom Merc decals (giant van sides)
      if (decal === 'stripes') {
        ctx.fillStyle = accentColor;
        // Giant diagonal delivery stripes
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.5, halfH * 0.5);
        ctx.lineTo(-halfW * 0.25, halfH * 0.5);
        ctx.lineTo(halfW * 0.25, -halfH * 0.5);
        ...
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.1, halfH * 0.5);
        ctx.lineTo(halfW * 0.1, halfH * 0.5);
        ctx.lineTo(halfW * 0.6, -halfH * 0.5);
        ...
        const grad = ctx.createLinearGradient(-halfW * 0.8, 0, halfW * 0.2, 0);
        grad.addColorStop(0, '#f97316');
        grad.addColorStop(1, '#facc15');
        ctx.fillStyle = grad;
        // Big retro panel van hot rod side flames
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.75, halfH * 0.3);
        ctx.bezierCurveTo(-halfW * 0.5, -halfH * 0.4, -halfW * 0.3, halfH * 0.1, halfW * 0.1, -halfH * 0.15);
        ctx.bezierCurveTo(-halfW * 0.1, halfH * 0.2, halfW * 0.25, halfH * 0.1, halfW * 0.4, halfH * 0.2);
        ctx.bezierCurveTo(0, halfH * 0.5, 0, halfH * 0.5, -halfW * 0.75, halfH * 0.3);
        ctx.closePath();
        ctx.fill();
      } else if (decal === 'lightning') {
        // Massive high voltage sign bolt on Merc panel sides
        ctx.strokeStyle = '#facc15';
        ...
        ctx.moveTo(-halfW * 0.5, -halfH * 0.5);
        ctx.lineTo(-halfW * 0.1, -halfH * 0.1);
        ctx.lineTo(-halfW * 0.35, -halfH * 0.05);
        ctx.lineTo(halfW * 0.15, halfH * 0.35);
        ctx.lineTo(-halfW * 0.1, halfH * 0.4);
        ctx.lineTo(halfW * 0.45, halfH * 0.75);
        ctx.stroke();
      } else if (decal === 'tribal') {
        // Swooping heavy metal style panel curves
        ctx.strokeStyle = '#020617';
        ctx.lineWidth = 4;
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.8, halfH * 0.2);
        ctx.bezierCurveTo(-halfW * 0.3, -halfH * 0.8, 0, halfH * 0.8, halfW * 0.5, -halfH * 0.1);
        ctx.stroke();
      } else if (decal === 'stars') {
        ctx.fillStyle = '#a855f7'; // Cosmic purple stars on Merc
        ...
        drawStar(-halfW * 0.5, -halfH * 0.5, 6);
        drawStar(-halfW * 0.2, -halfH * 0.1, 5);
        drawStar(halfW * 0.1, -halfH * 0.4, 5.5);
        drawStar(halfW * 0.3, halfH * 0.3, 5);
      }
      break;
    }

    case 'porsche': {
      // Custom Porsche decals (sleek, race lines, curved fastback)
      if (decal === 'stripes') {
        ctx.strokeStyle = accentColor;
        ctx.lineWidth = 2.8;
        ctx.beginPath();
        // Over-the-fender sleek curved endurance racing stripes
        ctx.arc(-halfW * 0.3, halfH * 0.3, halfW * 0.5, -Math.PI * 0.4, 0);
        ctx.stroke();

        ctx.beginPath();
        ctx.arc(-halfW * 0.3, halfH * 0.3, halfW * 0.58, -Math.PI * 0.45, 0);
        ctx.stroke();
      } else if (decal === 'flames') {
        const grad = ctx.createLinearGradient(-halfW * 0.4, 0, halfW * 0.6, 0);
        grad.addColorStop(0, '#f97316');
        grad.addColorStop(1, '#fb923c');
        ctx.fillStyle = grad;
        ctx.beginPath();
        // Sophisticated streamlined wind tunnel flames
        ctx.moveTo(-halfW * 0.4, halfH * 0.2);
        ctx.quadraticCurveTo(-halfW * 0.1, -halfH * 0.4, halfW * 0.3, -halfH * 0.1);
        ctx.quadraticCurveTo(halfW * 0.2, halfH * 0.05, halfW * 0.5, halfH * 0.1);
        ctx.quadraticCurveTo(0, halfH * 0.35, halfW * 0.35, halfH * 0.35);
        ctx.quadraticCurveTo(-halfW * 0.1, halfH * 0.45, -halfW * 0.4, halfH * 0.2);
        ctx.closePath();
        ctx.fill();
      } else if (decal === 'lightning') {
        // Laser energy line matching Porsche modern taillight bar
        ...
        ctx.lineTo(-halfW * 0.3, -halfH * 0.05);
        ctx.lineTo(-halfW * 0.1, -halfH * 0.05);
        ctx.lineTo(halfW * 0.15, halfH * 0.15);
        ctx.lineTo(halfW * 0.45, halfH * 0.15);
        ctx.stroke();
      } else if (decal === 'tribal') {
        // High luxury curves
        ...
        ctx.bezierCurveTo(-halfW * 0.2, -halfH * 0.5, halfW * 0.1, halfH * 0.5, halfW * 0.5, 0);
        ctx.stroke();
      } else if (decal === 'stars') {
        ctx.fillStyle = '#67e8f9'; // Cyan photon stars on Porsche
        ...
        drawStar(-halfW * 0.1, -halfH * 0.3, 4);
        drawStar(-halfW * 0.3, -halfH * 0.2, 3);
        drawStar(halfW * 0.2, halfH * 0.1, 4.5);
      }
      break;
    }

    case 'corvette': {
      // Custom Corvette C8.R decals (high-tech angular race supercar)
      if (decal === 'stripes') {
        ctx.fillStyle = accentColor;
        ctx.beginPath();
        // Aggressive sharp asymmetrical GT side-panel racing block
        ctx.moveTo(-halfW * 0.5, halfH * 0.45);
        ctx.lineTo(-halfW * 0.15, halfH * 0.45);
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.35, halfH * 0.05);
        ctx.lineTo(-halfW * 0.05, halfH * 0.05);
        ctx.stroke();
      } else if (decal === 'flames') {
        const grad = ctx.createLinearGradient(-halfW * 0.3, 0, halfW * 0.7, 0);
        grad.addColorStop(0, '#ec4899'); // Fuchsia/pink racing fire!
        grad.addColorStop(1, '#f43f5e');
        ctx.fillStyle = grad;
        ctx.beginPath();
        // Highly stylized supersonic jet exhaust plumes
        ctx.moveTo(-halfW * 0.3, halfH * 0.15);
        ctx.quadraticCurveTo(halfW * 0.1, -halfH * 0.35, halfW * 0.4, -halfH * 0.15);
        ctx.quadraticCurveTo(halfW * 0.25, halfH * 0.05, halfW * 0.6, halfH * 0.1);
        ctx.quadraticCurveTo(0, halfH * 0.4, halfW * 0.45, halfH * 0.3);
        ctx.quadraticCurveTo(-halfW * 0.1, halfH * 0.45, -halfW * 0.3, halfH * 0.15);
        ctx.closePath();
        ctx.fill();
      } else if (decal === 'lightning') {
        // High voltage neon green laser grids
        ctx.strokeStyle = '#4ade80';
        ctx.shadowColor = '#22c55e';
        ctx.shadowBlur = 9;
        ctx.lineWidth = 2.8;
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.7, -halfH * 0.05);
        ctx.lineTo(-halfW * 0.4, -halfH * 0.05);
        ctx.lineTo(-halfW * 0.5, halfH * 0.15);
        ctx.lineTo(-halfW * 0.1, halfH * 0.15);
        ctx.lineTo(-halfW * 0.2, halfH * 0.35);
        ctx.lineTo(halfW * 0.25, halfH * 0.35);
        ctx.stroke();
      } else if (decal === 'tribal') {
        // Supercar sharp vector razor wings
        ctx.fillStyle = '#090d16';
        ctx.beginPath();
        ctx.moveTo(-halfW * 0.5, halfH * 0.4);
        ctx.lineTo(-halfW * 0.1, halfH * 0.15);
        ctx.lineTo(-halfW * 0.25, halfH * 0.1);
        ctx.lineTo(halfW * 0.3, -halfH * 0.15);
        ctx.lineTo(-halfW * 0.1, halfH * 0.05);
        ctx.lineTo(halfW * 0.4, halfH * 0.3);
        ctx.closePath();
        ctx.fill();
      } else if (decal === 'stars') {
        ctx.fillStyle = '#fbbf24'; // Golden race-champion stars
        ctx.shadowColor = '#f59e0b';
        ctx.shadowBlur = 6;
        const drawStar = (x: number, y: number, r: number) => {
          ctx.beginPath();
        ...
        drawStar(-halfW * 0.5, halfH * 0.1, 5);
        drawStar(-halfW * 0.1, halfH * 0.05, 4.5);
        drawStar(halfW * 0.3, halfH * 0.2, 4);
      }
      break;
    }
  }

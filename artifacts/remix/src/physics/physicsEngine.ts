...
import { Ball, Car, Particle, SpecialShotType, Vector2D } from '../types/game';
...
export class PhysicsEngine {
  ...
  public update(
    ...
    // 6. Car vs Car Collision
    if (cars.length >= 2) {
      this.resolveCarCarCollision(cars[0], cars[1]);
    }
  ...
  private resolveCarGoalStructures(car: Car, wallX: number, isLeft: boolean) {
    ...
    if (inNetX) {
      // Landing on top of goal
      if (car.y + halfH >= topBarY && car.y < topBarY + 25 && car.vy >= 0) {
        car.y = topBarY - halfH;
        car.vy = 0;
        car.isGrounded = true;
      }
      // Hitting bottom of net structure
  ...
  public emitGoalBlast(x: number, y: number, team: 'blue' | 'orange') {
    const primaryColor = team === 'blue' ? '#3b82f6' : '#f97316';
    const secondaryColor = team === 'blue' ? '#60a5fa' : '#fb923c';

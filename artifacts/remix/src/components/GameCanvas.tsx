  ...
  const applyCarControls = (car: Car, controls: InputControls, dt: number) => {
    ...
    if (controls.jumpJustPressed) {
      ...
      if (car.isGrounded) {
        ...
        // Second Jump: Directional Flip Dodge!
        car.canFlip = false;
        car.jumpCount = 2;
        car.isFlipping = true;
        car.flipProgress = 0;

        // Determine dodge impulse direction from steering
        let dirX = controls.steerX;
        let dirY = controls.steerY;

        if (Math.hypot(dirX, dirY) < 0.2) {
          // If no directional input held, dodge in facing direction
          dirX = (car.facing || 1) * Math.cos(car.angle);
          dirY = -0.5;
        }
        ...
        car.vx = dirX * PHYSICS.car.flipImpulse;
        car.vy = dirY * PHYSICS.car.flipImpulse;

        // Exactly one full clean 360° spin during flip duration
        car.angularVelocity = (dirX >= 0 ? 1 : -1) * (Math.PI * 2) / PHYSICS.car.flipDuration;
  ...
  useEffect(() => {
    ...
    const gameLoop = (currentTime: number) => {
      ...
      if (matchState.status === 'playing') {
        ...
        // Player 2 / Bot controls
        if (cars[1]) {
          if (gameMode === 'local-2p') {
            applyCarControls(cars[1], p2InputsRef.current, dt);
          } else {
            const botControls = botRef.current.getControls(cars[1], ball, botDifficulty, dt);
        ...
        if (goalScored) {
          ...
          // Goal explosion particles at net mouth
          ...
          physicsRef.current.emitGoalBlast(netMouthX, netMouthY, goalScored);

          // Blast impulse on cars
          for (const car of cars) {
            const cdx = car.x - netMouthX;
            const cdy = car.y - netMouthY;
            const cdist = Math.hypot(cdx, cdy) || 1;
            car.vx += (cdx / cdist) * 900;
            car.vy += (cdy / cdist) * 900 - 300;
          }
        ...
        // Match Clock
        if (!isTraining && !matchState.isOvertime) {
      ...
      if (canvas && container) {
        ...
        if (ctx) {
          ...
          rendererRef.current.updateCamera(cars, ball, logicalW, logicalH, dt, framingMode);
